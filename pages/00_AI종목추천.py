...
tab1, tab2, tab3, tab4, tab5 = st.tabs([
    "📊 수익률 비교", "📉 개별 차트", "🏆 성과 순위", "📋 상세 지표", "🤖 AI 종목 추천"
])
...
# ════════════════════════════════════════════════════════════════════════════════
with tab5:
    # ── Extra CSS for AI tab ──
    st.markdown("""
    <style>
      .ai-banner {
        background: linear-gradient(135deg, #0d1f3c 0%, #0f1f2e 50%, #0a1628 100%);
        border: 1px solid #1e3a5f;
        border-radius: 14px;
        padding: 22px 28px;
        margin-bottom: 20px;
      }
      .ai-banner h3 { color: #38bdf8; margin: 0 0 6px; font-size: 1.1rem; }
      .ai-banner p  { color: #64748b; margin: 0; font-size: 0.85rem; line-height: 1.6; }
      .risk-chip {
        display: inline-block; border-radius: 20px; padding: 5px 16px;
        font-size: 0.78rem; font-weight: 600; cursor: pointer; margin: 4px;
      ...
      .ai-result-box {
        background: linear-gradient(135deg, #0d1221 0%, #0f172a 100%);
        border: 1px solid #1e3a5f;
        border-radius: 14px;
        padding: 24px 28px;
        margin-top: 16px;
      ...
      .ai-result-box h4 { color: #38bdf8; margin-top: 18px; margin-bottom: 6px; }
      .ai-result-box strong { color: #f1f5f9; }
    </style>
    """, unsafe_allow_html=True)

    st.markdown("""
    <div class="ai-banner">
      <h3>🤖 Claude AI 종목 추천 엔진</h3>
      <p>현재 선택된 종목들의 <b>실시간 주가 데이터</b>(수익률·변동성·모멘텀·샤프지수)를 Claude가 분석하여<br>
      투자 성향에 맞는 종목을 추천합니다. API 키는 Streamlit Secrets에 저장하세요.</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    try:
        ...
        api_key = st.text_input(
            ...
            type="password",
            placeholder="sk-ant-...",
            help="Streamlit Cloud 배포 시 Secrets에 ANTHROPIC_API_KEY를 설정하세요.",
        )
    ...
    with col_l:
        st.markdown("**📌 투자 성향 설정**")
        risk_level = st.select_slider(
            "리스크 허용 수준",
            options=["매우 보수적", "보수적", "중립", "공격적", "매우 공격적"],
            value="중립",
        )
        invest_period = st.radio(
            "투자 기간",
            ["단기 (1~3개월)", "중기 (6개월~1년)", "장기 (1년 이상)"],
            horizontal=True,
        )
    with col_r:
        st.markdown("**🎯 추천 목표**")
        goals = st.multiselect(
            "관심 투자 목표 (복수 선택)",
            ["고수익 추구", "안정적 배당", "변동성 최소화", "모멘텀 투자", "가치 투자", "성장주 투자"],
            default=["고수익 추구", "변동성 최소화"],
        )
        extra_prompt = st.text_area(
            "추가 요청사항 (선택)",
            placeholder="예: 반도체 관련주 위주로, 배당 수익률 2% 이상 포함해줘",
            height=90,
        )

    st.markdown("""
    <div class="disclaimer">
      ⚠️ <b>투자 주의사항</b>: 본 AI 추천은 과거 데이터 기반의 참고 정보이며 투자 권유가 아닙니다.
      실제 투자 결정은 반드시 본인의 판단과 책임 하에 이루어져야 합니다.
    </div>
    """, unsafe_allow_html=True)

    st.markdown("<br>", unsafe_allow_html=True)
    run_ai = st.button("🚀 AI 분석 시작", use_container_width=True, type="primary")

    # ── Build stock data summary for Claude ──
    def build_stock_summary(all_tickers, all_data, kr_tickers, period_label):
        rows = []
        for name, ticker in all_tickers.items():
            df = all_data.get(ticker)
            if df is None or df.empty:
            ...
            # Momentum: recent 20d vs prior
            if len(close) >= 40:
                mom = float((close.iloc[-1] / close.iloc[-20] - 1) * 100)
            else:
                mom = ret
            sharpe = round(ret / vol_, 2) if vol_ > 0 else 0
            market = "한국(KOSPI)" if ticker in kr_tickers.values() else "미국(NYSE/NASDAQ)"
            rows.append({
                "종목명": name, "티커": ticker, "시장": market,
                f"기간수익률({period_label})": f"{ret:+.2f}%",
                "연환산변동성": f"{vol_:.2f}%",
                "최근20일모멘텀": f"{mom:+.2f}%",
                "샤프지수": sharpe,
                "현재가": f"{price:,.2f}",
    ...
    if run_ai:
        if not api_key:
            ...
            stock_rows = build_stock_summary(all_tickers, all_data, kr_tickers, period_label)
            stock_json = json.dumps(stock_rows, ensure_ascii=False, indent=2)
            goals_str  = ", ".join(goals) if goals else "없음"

            system_prompt = """당신은 퀀트 분석과 글로벌 주식 투자에 정통한 시니어 포트폴리오 매니저입니다.
사용자가 제공한 실제 주가 데이터(수익률, 변동성, 모멘텀, 샤프지수)를 기반으로 분석하세요.
응답은 반드시 한국어로 작성하며, 구체적인 데이터 근거를 제시하고 명확한 종목 추천을 해주세요.
투자 권유가 아닌 데이터 기반 분석임을 명시하세요."""
...
**투자 기간**: {invest_period}
**투자 목표**: {goals_str}
**추가 요청**: {extra_prompt if extra_prompt else "없음"}
**조회 기간**: {period_label}
...
## 요청사항
위 실제 데이터를 바탕으로 다음을 수행해주세요:

1. **📊 시장 현황 요약** (2~3문장, 한국/미국 시장 전반 흐름)
2. **🏆 TOP 3 추천 종목** (각 종목별로):
   - 추천 이유 (데이터 수치 인용)
   - 리스크 요인
   - 적정 비중 제안 (%)
3. **⚠️ 주의 종목** (리스크가 높거나 성과가 저조한 종목 1~2개, 이유 포함)
4. **💼 포트폴리오 구성 제안** (한국/미국 비중, 섹터 분산 등)
5. **📌 한 줄 결론** (투자 성향에 맞는 핵심 메시지)"""

            with st.spinner("🤖 Claude AI가 종목 데이터를 분석 중입니다..."):
                try:
                    ...
                    with client.messages.stream(
                        ...
                        for text_chunk in stream.text_stream:
                            ...
                            result_placeholder.markdown(
                                f'<div class="ai-result-box">{full_text}▌</div>',
                                unsafe_allow_html=True,
                            )
